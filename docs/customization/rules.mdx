...
title: "Rules"
sidebarTitle: "Rules"
description: "Define specific instructions and coding standards for Codie."
---

Rules are markdown files that provide persistent instructions across all conversations. Instead of repeating the same preferences every time you start a new task, rules let you define them once and have Codie follow them automatically.

Use rules when you want Codie to:
...
Codie recognizes rules from multiple sources, so you can use existing rule files from other tools:

| Rule Type | Location | Description |
|-----------|----------|-------------|
| Codie Rules | `.codevibe/rules/` | Primary rule format |
...
**Global rules** go in your system's Codie Rules directory. Use these for personal preferences that apply across all projects. Codie also reads cross-tool global AGENTS instructions from `~/.agents/AGENTS.md`.
...
Codie processes all `.md` and `.txt` files inside `.codevibe/rules/`, combining them into a unified set of rules. Numeric prefixes (like `01-coding.md`) help organize files but are optional. Existing `.clinerules/` files are still detected for compatibility.

When both workspace and global rules exist, Codie combines them. Workspace rules take precedence when they conflict with global rules. See [Storage Locations](/getting-started/config#storage-locations) for more guidance.
...
<Note>
Codie also checks `~/Documents/CodeVibe/Rules`. Legacy `~/.cline/rules` and older Documents-based rule directories are still read for migration.
</Note>
...
<Steps>
  <Step title="Open the Rules menu">
    Click the scale icon at the bottom of the Codie panel, to the left of the model selector.
  </Step>
...
You can also use the [`/newrule` slash command](/core-workflows/using-commands#newrule) to have Codie create a rule interactively.
...
Codie reads rules as context, so formatting matters. Headers help Codie understand the scope of each instruction. Bullet points make individual requirements clear. Code examples show exactly what you want.

### Best Practices

**Be specific, not vague.** "Use descriptive variable names" is too broad. "Use camelCase for variables, PascalCase for classes, UPPER_SNAKE for constants" gives Codie something concrete to follow.

**Include the why.** When a rule might seem arbitrary, explain the reason. "Don't modify files in /legacy (this code is scheduled for removal in Q2)" helps Codie make better decisions in edge cases.
...
**Keep rules current.** Outdated rules confuse Codie and waste context. If a constraint no longer applies, remove it. If your tech stack changes, update the rules.
...
**Invalid YAML**: If frontmatter can't be parsed, Codie fails open. The rule activates with raw content visible to help debugging.

### What Counts as "Current Context"

Codie evaluates rules based on:
...
4. **Edited files**: Files Codie has created, modified, or deleted during the task
5. **Pending operations**: Files Codie is about to edit

Conditional rules can activate on your first message, when relevant files are open, or mid-task when Codie starts working with matching files.
