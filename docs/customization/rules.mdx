...
title: "Rules"
sidebarTitle: "Rules"
description: "Define specific instructions and coding standards for CodeVibe."
---

Rules are markdown files that provide persistent instructions across all conversations. Instead of repeating the same preferences every time you start a new task, rules let you define them once and have CodeVibe follow them automatically.

Use rules when you want CodeVibe to:
...
CodeVibe recognizes rules from multiple sources, so you can use existing rule files from other tools:

| Rule Type | Location | Description |
|-----------|----------|-------------|
| CodeVibe Rules | `.codevibe/rules/` | Primary rule format |
| Legacy rules | `.clinerules/` | Migration-compatible rule format |
...
**Workspace rules** go in `.codevibe/rules/` at your project root. Use these for team standards, project-specific constraints, and anything you want to share with collaborators via version control.

**Global rules** go in your system's CodeVibe Rules directory. Use these for personal preferences that apply across all projects. CodeVibe also reads cross-tool global AGENTS instructions from `~/.agents/AGENTS.md`.
...
CodeVibe processes all `.md` and `.txt` files inside `.codevibe/rules/`, combining them into a unified set of rules. Numeric prefixes (like `01-coding.md`) help organize files but are optional. Existing `.clinerules/` files are still detected for compatibility.

When both workspace and global rules exist, CodeVibe combines them. Workspace rules take precedence when they conflict with global rules. See [Storage Locations](/getting-started/config#storage-locations) for more guidance.

### Global Rules Directory

| Operating System | Default Location |
|------------------|------------------|
| Windows | `%USERPROFILE%\.codevibe\rules` |
| macOS | `~/.codevibe/rules` |
| Linux/WSL | `~/.codevibe/rules` |

<Note>
CodeVibe also checks `~/Documents/CodeVibe/Rules`. Legacy `~/.cline/rules` and older Documents-based rule directories are still read for migration.
</Note>
...
<Steps>
  <Step title="Open the Rules menu">
    Click the scale icon at the bottom of the CodeVibe panel, to the left of the model selector.
  </Step>
...
You can also use the [`/newrule` slash command](/core-workflows/using-commands#newrule) to have CodeVibe create a rule interactively.
...
CodeVibe reads rules as context, so formatting matters. Headers help CodeVibe understand the scope of each instruction. Bullet points make individual requirements clear. Code examples show exactly what you want.

### Best Practices

**Be specific, not vague.** "Use descriptive variable names" is too broad. "Use camelCase for variables, PascalCase for classes, UPPER_SNAKE for constants" gives CodeVibe something concrete to follow.

**Include the why.** When a rule might seem arbitrary, explain the reason. "Don't modify files in /legacy (this code is scheduled for removal in Q2)" helps CodeVibe make better decisions in edge cases.
...
**Keep rules current.** Outdated rules confuse CodeVibe and waste context. If a constraint no longer applies, remove it. If your tech stack changes, update the rules.
...
Add YAML frontmatter to the top of any rule file in your `.codevibe/rules/` directory:
...
**Invalid YAML**: If frontmatter can't be parsed, CodeVibe fails open. The rule activates with raw content visible to help debugging.

### What Counts as "Current Context"

CodeVibe evaluates rules based on:
...
4. **Edited files**: Files CodeVibe has created, modified, or deleted during the task
5. **Pending operations**: Files CodeVibe is about to edit

Conditional rules can activate on your first message, when relevant files are open, or mid-task when CodeVibe starts working with matching files.
