...
title: "Skills"
sidebarTitle: "Skills"
description: "Modular instruction sets that extend CodeVibe's capabilities for specific tasks."
---

Skills are modular instruction sets that extend CodeVibe's capabilities for specific tasks. Each skill packages detailed guidance, processes, and optional resources that CodeVibe loads only when relevant to your request.

Install multiple skills and CodeVibe only loads what it needs. A deployment skill stays dormant until you ask about deploying. Unlike [rules](/customization/cline-rules) (which are always active), skills load on-demand so they don't consume context when you're working on something unrelated.
...
When you send a message, CodeVibe sees a list of available skills with their descriptions. If your request matches a skill's description, CodeVibe activates it using the `use_skill` tool, which loads the full instructions from SKILL.md.
...
3. CodeVibe triggers that skill and loads its `SKILL.md` instructions.
...
Detailed instructions for CodeVibe to follow when this skill is activated.
...
- `description` tells CodeVibe when to use this skill (max 1024 characters)

## Creating a Skill

<Steps>
  <Step title="Open the Skills menu">
    Click the scale icon at the bottom of the CodeVibe panel, to the left of the model selector. Switch to the Skills tab.
  </Step>
  <Step title="Create a new skill">
    Click "New skill..." and enter a name for your skill (e.g., `aws-deploy`). CodeVibe creates a skill directory with a template `SKILL.md` file.
  </Step>
...
You can also create skills manually by creating the directory structure in your file system. Place skill directories in `.codevibe/skills/` (workspace) or `~/.codevibe/skills/` (global) and CodeVibe will detect them automatically. Legacy `.cline/skills/` and `.clinerules/skills/` locations remain supported for migration.

Put the important information first in your SKILL.md. CodeVibe reads the file sequentially, so front-load the common cases. Use clear section headers like "## Error Handling" or "## Configuration" so CodeVibe can scan for relevant sections.
...
The description determines when CodeVibe activates the skill. A vague description means the skill won't trigger when you expect it to.
...
Keep SKILL.md under 5k tokens. If your skill needs more content, split it into separate files in a `docs/` directory and reference them from the main instructions. CodeVibe loads referenced files only when needed.
...
Project skills:
- `.codevibe/skills/` (recommended)
- `.cline/skills/` (legacy)
- `.clinerules/skills/` (legacy)
- `.claude/skills/`

Global skills:
- `~/.codevibe/skills/` (macOS/Linux)
- `C:\Users\USERNAME\.codevibe\skills\` (Windows)

When a global skill and project skill have the same name, the global skill takes precedence. This lets you keep general-purpose skills globally while using project-specific skills in `.codevibe/skills/` so the whole team can use them.

Version control your project skills by committing `.codevibe/skills/`. Your team can share, review, and improve them together.

## Bundling Supporting Files

Skills can include additional files that CodeVibe accesses only when needed.
...
A deployment skill might have `docs/aws.md`, `docs/gcp.md`, and `docs/azure.md`. CodeVibe loads only the relevant platform guide based on your request.
...
A project setup skill could include `templates/dockerfile`, `templates/docker-compose.yml`, and `templates/.env.example` that CodeVibe customizes for each new project.
...
CodeVibe reads documentation files using `read_file` when the instructions reference them. Scripts can be executed directly, and only the script's output enters the context window.
...
Skills transform CodeVibe from a general-purpose assistant into a specialist that knows your domain. Start with one skill for a task you repeat often, test it, and iterate on the description until it triggers reliably.
