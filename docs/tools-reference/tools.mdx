...
title: "Tools"
sidebarTitle: "Tools"
description: "Reference for CodeVibe's built-in tool system, aligned with SDK tool architecture."
---

CodeVibe tools are executable functions the model can call while working. The model decides which tool to call, CodeVibe runs it, then returns results back to the model.

## Built-In Tools

When running through the CodeVibe runtime, these built-ins are available:
...
CodeVibe can run tools with approval or auto-approval, depending on settings/policies. Typical patterns:
...
CodeVibe can also call tools discovered from MCP servers configured in `.codevibe/mcp.json`; legacy `.cline/mcp.json` is still supported for existing projects.

MCP tools are loaded alongside built-ins, so CodeVibe can use both local tools and external integrations in one task.
...
Some older docs/examples reference XML-style names like `read_file`, `replace_in_file`, or `execute_command`. The current CodeVibe runtime uses the built-in tool names listed above (`read_files`, `apply_patch`, `bash`, etc.).
