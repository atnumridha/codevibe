...
| Plan files and native editor | Focused tests passed on 2026-06-15 for `PlanStorageService`, `PlanModeRespondHandler`, `AttemptCompletionHandler plan mode`, and `Hostbridge - Workspace - searchWorkspaceText`. Plan responses persist `.plan.md` files, Plan-mode `attempt_completion` now also persists and opens the same native `.plan.md` artifact instead of leaving only a plain completion row, the webview renders that payload as the plan card with open/build controls, the native opener targets `codevibe.planEditor`, the native Plans view refreshes on storage changes, the actual `*.plan.md` filename is visible in the plan card, and workspace fallback writes `.cursor/.gitignore` with `plans/`. `*.plan.md` now defaults to the `codevibe.planEditor` custom editor, which opens as a rendered local plan canvas instead of raw text, packages a standalone local Mermaid renderer under `assets/plan-editor/`, includes a built-in SVG fallback for common flowcharts, exposes `Build`, `Build Selected`, `Build Locally`, and `Build in Parallel`, keeps cloud build excluded, and leaves raw Markdown behind an explicit toggle. Slow installed VSIX e2e runs on 2026-06-15 verified real installed-extension plan creation, frontmatter todos, Mermaid plan body persistence, native custom-editor association, rendered-canvas expectation, visible SVG diagram rendering, local build actions, absence of cloud build action, and `Build Locally` materializing a missing `.plan.md` from a visible Plan card before opening `codevibe.planEditor`. | Focused and installed VSIX guards passed |
| Retrieval/indexed search | `Hostbridge - Workspace - searchWorkspaceText` test passed on 2026-06-15 and asserts VS Code `findTextInFiles` is used without enumerating/reading candidate files before search. The fallback path now uses bundled ripgrep JSON compact matches instead of `vscode.workspace.findFiles` plus `workspace.fs.readFile`, with a focused guard proving fallback file enumeration and file reads stay at zero. The installed VSIX e2e also passed on 2026-06-15 and verified the packaged extension calls VS Code native `findTextInFiles` once, respects a `*.ts` include pattern, returns a compact line match, and does not hit the fallback `findFiles` or `workspace.fs.readFile` path. | Focused and installed VSIX guards passed |
...
| Installed VS Code visual validation | The active VS Code window rendered the `CODIE AGENT` sidebar and composer after reload; screenshot evidence: `/private/tmp/codie-code-app-activated.png`. `npm --prefix apps/vscode run test:e2e:optimal -- src/test/e2e/native-chat-installed.test.ts --project "e2e tests"` passed on 2026-06-15 after installing `apps/vscode/dist/e2e.vsix` into VS Code. Additional slow installed UI runs with `CODEVIBE_E2E_SLOW_UI=true npx playwright test src/test/e2e/native-chat-installed.test.ts --project "e2e tests"` passed on 2026-06-15 for the composer typed-input smoke, native plan editor canvas, `Build Locally` missing-plan materialization, and browser automation transcript cases. The installed-extension guard verifies the Codie native chat agent, command exposure, native session opening, packaged prompt contributions, packaged skill contributions, `codevibe-agent` session targeting, packaged prompt/skill files on disk, ChatGPT for Codie as the default provider/model path, local `.codex/auth.json` import with account/model state surfaced and secrets redacted, isolated local plan home, real `.plan.md` creation, native `codevibe.planEditor` tab opening/rendered canvas expectation, VS Code-native text search before file reads, Cursor-compatible `.cursor/sandbox.json` policy import, sandboxed default terminal run mode, elevated inline terminal requests, redirect blocking, git-write denies, visible sandbox command approval controls in the installed webview, visible browser automation transcript rendering for launch/snapshot/screenshot/click/type/close with screenshot and console logs, Cursor-compatible deeplink routing through installed local VS Code handlers, MCP install plus OAuth next-action/callback routing, background-agent launch request routing, automation ingest routing, plugin add/replace routing, PR/rule/command/git/settings/glass/createchat/prompt route confirmations, disabled compatible-deeplink blocking, secret redaction in route confirmations, and composer launch. Native Chat request-handler invocation/build handoff, full MCP hub UI, and full background-agent worktree/session execution still need recorded installed-VS-Code evidence. | Installed VSIX guard passed; broader installed-flow evidence still needed |
| Local RC install | `node apps/vscode/scripts/package-github-vsix.mjs --out-dir /private/tmp/codevibe-vsix --install --verify-install` passed on 2026-06-15. It packaged `/private/tmp/codevibe-vsix/codevibe-3.88.77.vsix`, installed `atnumridha.codevibe@3.88.77` into local VS Code, confirmed proposed API argv was already enabled, cleaned legacy sidebar view state, verified the default install, and verified an isolated smoke install. | Local install verified |
...
- Browser automation can launch, snapshot, click, type, and screenshot with configured privacy controls. Automated installed-VSIX evidence now covers the visible launch URL, screenshot frame, console snapshot section, click action, and type action in the packaged Codie webview.
...
- Cursor sandbox policy imports from `.cursor/sandbox.json`. Automated installed-VSIX evidence now covers import, default sandbox terminal mode, elevated inline terminal requests, redirect blocking, git-write denies, and visible sandbox command approval controls.
...
Focused local evidence from `release:cursor-parity:evidence:fast` passed on 2026-06-15 for retrieval/indexing, MCP install/OAuth, sandbox policy, deeplinks, NDJSON ingest, background agents, browser tools, and Mermaid planning. The installed VSIX native chat e2e also passed on 2026-06-15 for direct packaged prompt/skill contribution discovery, native `.plan.md` editor opening, ChatGPT for Codie local auth/default model state, VS Code-native text search before file reads, Cursor-compatible sandbox policy/inline terminal run modes, visible browser automation transcript rendering, and Cursor-compatible deeplink/MCP/background-agent route handling through installed local VS Code handlers. Additional focused unit evidence on 2026-06-15 confirms Plan-mode completion produces a native `.plan.md` payload and fallback search uses compact ripgrep matches without VS Code candidate file reads. The remaining gap for this section is broader installed-VS-Code runtime coverage, not local unit/hub coverage.
