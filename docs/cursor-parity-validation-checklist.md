...
| Installed VS Code visual validation | The active VS Code window rendered the `CODIE AGENT` sidebar and composer after reload; screenshot evidence: `/private/tmp/codie-code-app-activated.png`. `npm --prefix apps/vscode run test:e2e:optimal -- src/test/e2e/native-chat-installed.test.ts --project "e2e tests"` passed on 2026-06-15 after installing `apps/vscode/dist/e2e.vsix` into VS Code. A focused rerun of the installed plan-editor case also passed on 2026-06-15 after packaging the Mermaid asset into the VSIX and asserting the actual editor-frame text contains the rendered local plan canvas plus `Build`, `Build Selected`, `Build Locally`, `Build in Parallel`, and `Raw Markdown` with no cloud build action. The installed-extension guard verifies the Codie native chat agent, command exposure, native session opening, packaged prompt contributions, packaged skill contributions, `codevibe-agent` session targeting, packaged prompt/skill files on disk, ChatGPT for Codie as the default provider/model path, local `.codex/auth.json` import with account/model state surfaced and secrets redacted, isolated local plan home, real `.plan.md` creation, native `codevibe.planEditor` tab opening/rendered canvas expectation, VS Code-native text search before file reads, Cursor-compatible `.cursor/sandbox.json` policy import, sandboxed default terminal run mode, elevated inline terminal requests, redirect blocking, git-write denies, visible sandbox command approval controls in the installed webview, Cursor-compatible deeplink routing through installed local VS Code handlers, MCP install plus OAuth next-action/callback routing, background-agent launch request routing, automation ingest routing, plugin add/replace routing, PR/rule/command/git/settings/glass/createchat/prompt route confirmations, disabled compatible-deeplink blocking, secret redaction in route confirmations, and composer launch. Native Chat request-handler invocation/build handoff, browser automation, full MCP hub UI, and full background-agent worktree/session execution still need recorded installed-VS-Code evidence. | Installed VSIX guard passed; broader installed-flow evidence still needed |
