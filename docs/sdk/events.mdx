...
- `AgentEvent` / `CoreSessionEvent` from `@cline/core`, delivered through CodieCore session adapters and `codie.subscribe(listener)`.
...
`AgentEvent` is the host-facing event shape used by CodieCore session orchestration. For direct `Agent` usage, prefer `agent.subscribe(...)` and `AgentRuntimeEvent`.
...
## CodieCore Session Events

Use `codie.subscribe()` for session-level events:

```typescript
const unsubscribe = codie.subscribe((event) => {
...
```typescript
const unsubscribe = codie.subscribe(listener, { sessionId })
```
...
For a complete working example of streaming agent events to a browser via SSE, see the [multi-agent example](https://github.com/atnumridha/codevibe/tree/main/apps/examples/multi-agent). It spawns multiple agents in parallel and streams each agent's events to separate UI cards.
