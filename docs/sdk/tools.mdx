...
CodieCore can enable the built-in tool suite:
...
For complete examples of tools in action, see the [cli-agent](https://github.com/atnumridha/codevibe/tree/main/apps/examples/cli-agent) (shell tool) and [code-review-bot](https://github.com/atnumridha/codevibe/tree/main/apps/examples/code-review-bot) (multiple tools with completion lifecycle).
...
With CodieCore, custom tools are passed as `extraTools` in session config:
...
Tools work alongside MCP. CodieCore can load MCP settings through its runtime/config extension path. MCP tools are registered alongside built-in and custom tools when MCP support is enabled for the session.
