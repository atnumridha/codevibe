---
title: "CodieCore"
sidebarTitle: "CodieCore"
description: "The full Codie harness: built-in tools, sessions, approvals, scheduling, and hub support."
---

CodieCore is the full Codie harness as a programmable runtime. It gives you everything Codie ships out of the box: built-in tools for files, shell, search, and web; session persistence and message history; tool approval callbacks; local, hub, and remote backends; scheduling and automation APIs; and plugin support.

Import `CodieCore` from `@cline/sdk` for new applications. The compatibility export `ClineCore` and the `@cline/*` package scope remain supported for existing integrations and upstream-compatible packages. Under the hood, CodieCore uses the `Agent` class from `@cline/agents`. You can use `Agent` directly if you want to skip the harness and wire everything yourself: your own tools, your own persistence, your own lifecycle.

## When to use which

| Need | Use |
|------|-----|
| Sessions, persistence, message history | CodieCore |
| Built-in tools for files, shell, search, web fetch | CodieCore |
| Hub/remote runtime | CodieCore |
| Scheduling or event automation | CodieCore |
| Multi-agent teams | CodieCore |
...
CodieCore wraps runtime execution with application features:
...
const codie = await CodieCore.create({
  clientName: "my-app",
  backendMode: "auto",
})

const session = await codie.start({
...
| Method | Purpose |
|--------|---------|
| `CodieCore.create(options)` | Create the runtime |
...
See [CodieCore reference](/sdk/reference/cline-core) for exact signatures.
...
CodieCore stores session manifests and messages as files. A session result includes:
...
`Agent` (also exported as `AgentRuntime`) is the stateless primitive that CodieCore builds on. Use it directly when you want full control or don't need the harness.
