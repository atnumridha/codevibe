...
Teams are a CodieCore feature. Enable them in the session config:

```typescript
import { CodieCore } from "@cline/sdk"

const codie = await CodieCore.create({ clientName: "team-app" })
...
```bash
codevibe --team-name auth-sprint "Continue -- pick up incomplete tasks"
```
...
```bash
# Start a new team
codevibe --team-name auth-sprint "Plan and implement user auth with tests"

# Resume work
codevibe --team-name auth-sprint "What's the status? Continue with unfinished tasks."

# Different team for a different workstream
codevibe --team-name perf-sprint "Profile the API endpoints and optimize the slowest 3"
```
