...
To load this plugin from a file in CodieCore, pass its path in `pluginPaths`:
...
```bash
codevibe plugin install https://github.com/your-org/codevibe-github-plugin.git
```
...
Each bundled skill follows the same directory format as any other [Codie skill](/customization/skills). When the plugin is installed or loaded through `pluginPaths`, Codie discovers those skills automatically.
