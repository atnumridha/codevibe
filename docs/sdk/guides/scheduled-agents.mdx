...
Scheduled agents rely on the [hub-spoke architecture](/sdk/architecture/hub-spoke) that is part of CodieCore. The hub runs as a background process on your machine. It starts automatically when needed and persists schedules across restarts.

## Creating Schedules Programmatically with Codie SDK
...
await codie.automation.start()

// Use codie.automation to reconcile specs, ingest events, and list runs.
```

## Schedule Wizard with Codie CLI

The easiest way to schedule tasks is to use the Codie CLI, if you've already installed it with `npm i -g codevibe`.

Run `codevibe schedule` to open an interactive menu for creating and managing schedules, browsing execution history, and viewing performance statistics.

## Creating Schedules with Flags

```bash
# Daily at 9 AM on weekdays
codevibe schedule create "PR summary" \
  --cron "0 9 * * MON-FRI" \
...
# Every 6 hours
codevibe schedule create "Health check" \
  --cron "0 */6 * * *" \
  --prompt "Run the test suite and report any failures" \
  --workspace /path/to/project

# Monday mornings
codevibe schedule create "Weekly digest" \
  --cron "0 8 * * MON" \
...
# Pause a schedule
codevibe schedule pause <schedule-id>

# Resume a paused schedule
codevibe schedule resume <schedule-id>

# Delete a schedule
codevibe schedule delete <schedule-id>

# View execution history
codevibe schedule executions <schedule-id>
```
...
```bash
codevibe schedule create "Standup prep" \
  --cron "0 8 * * MON-FRI" \
...
```bash
codevibe schedule create "Dependency check" \
  --cron "0 10 * * MON" \
...
```bash
codevibe schedule create "Code health" \
  --cron "0 6 * * MON" \
...
codevibe connect telegram -k $BOT_TOKEN

# Schedule an agent whose output gets sent to Telegram
codevibe schedule create "Morning briefing" \
  --cron "0 8 * * *" \
