...
`CodieCore.subscribe(listener, options?)` emits session-level events from the runtime host. The stable `ClineCore` compatibility export exposes the same method for existing integrations.
