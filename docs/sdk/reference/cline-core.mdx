...
description: "API reference for CodieCore, exported from @cline/sdk and @cline/core."
---

CodieCore is the product name and preferred export for the full Codie runtime harness. The compatibility API `ClineCore` remains available from `@cline/sdk` / `@cline/core` for existing imports and upstream-compatible package names.

```typescript
import { CodieCore } from "@cline/sdk"
```

## `CodieCore.create(options)`
...
| `automation` | `boolean \| CodieCoreAutomationOptions` | Enable automation APIs |
...
When `automation` is enabled, use `codie.automation` to start/stop automation services, reconcile specs, ingest events, and list events/specs/runs.
