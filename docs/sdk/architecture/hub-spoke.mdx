...
<Steps>
  <Step title="Client discovers or starts the hub">
    The hub is a singleton daemon per machine. If one isn't running, CodieCore starts it automatically. Discovery uses lock files at `~/.codevibe/locks/hub/owners/`; legacy `~/.cline` lock locations remain supported during migration.
  </Step>
...
CodieCore selects the execution mode based on configuration:
...
The hub starts automatically when CodieCore needs it. You can also manage it manually:

```bash
codevibe hub start      # Start the hub daemon
codevibe hub stop       # Stop it
codevibe hub status     # Check if running
codevibe hub ensure     # Start if not running, return URL
```

The hub listens on `127.0.0.1:25463` by default and logs to `~/.codevibe/logs/hub-daemon.log`.
