...
<Steps>
  <Step title="Client discovers or starts the hub">
    The hub is a singleton daemon per machine. If one isn't running, `ClineCore` starts it automatically. Discovery uses lock files at `~/.codevibe/locks/hub/owners/`; legacy `~/.cline` lock locations remain supported during migration.
  </Step>
