...
Published package names intentionally remain under the stable compatibility scope `@cline/*`. CodieCore is the product-facing name and preferred export for the full runtime harness; `ClineCore` remains as a stable compatibility export.
...
| Export | Description |
|--------|-------------|
| `CodieCore` | Preferred export for the CodieCore runtime entry point |
| `CodieCoreOptions` | Constructor options for CodieCore |
| `CodieCoreStartInput` | Session start input for CodieCore |
| `ClineCore` | Stable compatibility export for existing imports |
...
`@cline/sdk` re-exports everything from `@cline/core`. Install `@cline/agents` or `@cline/llms` directly only if you need lower-level control. There are no `@codie/*` package aliases today.
