...
CodieCore supports plugin module paths via `pluginPaths` in session config:
...
Plugins can also be installed from file URLs, npm, git, or local paths using `codevibe plugin install`. See [Plugins](/customization/plugins) for install commands, the manifest format, and directory layout.
