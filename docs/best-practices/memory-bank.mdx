...
2. Add them to a [CodeVibe Rules file](/customization/cline-rules), such as the legacy-compatible `.clinerules/memory-bank.md`
...
- Use [CodeVibe Rules](/customization/cline-rules) to store the Memory Bank instructions per project
...
Either works. Custom instructions apply globally across all projects. A [CodeVibe Rules file](/customization/cline-rules) is project-specific and stored in your repo, which makes it easy to share with collaborators. You can also use [conditional rules](/customization/cline-rules#conditional-rules) to activate Memory Bank instructions only when working with `memory-bank/` files.
