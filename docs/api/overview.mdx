...
Welcome to the Codie API documentation. Use the same models that power the Codie extension and CLI from any language, framework, or tool that speaks the OpenAI format.

## What is the Codie API?

The Codie API is an OpenAI-compatible Chat Completions endpoint. You authenticate once with a Codie API key and get access to models from Anthropic, OpenAI, Google, and more through a single base URL. No need to manage separate keys for each provider.

```
Your App  →  Codie API  →  Anthropic / OpenAI / Google / etc.
```
...
<CardGroup cols={2}>
  ...
  <Card title="Code Examples" icon="code" href="/api/sdk-examples">
    Ready-to-copy examples for Python, Node.js, curl, and the Codie CLI.
  </Card>
</CardGroup>
...
Get your API key from the Codie account console at [app.codie.dev](https://app.codie.dev) (Settings > API Keys), then follow the [Getting Started](/api/getting-started) guide.
