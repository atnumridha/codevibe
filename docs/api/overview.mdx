...
Welcome to the CodeVibe API documentation. Use the same models that power the CodeVibe extension and CLI from any language, framework, or tool that speaks the OpenAI format.

## What is the CodeVibe API?

The CodeVibe API is an OpenAI-compatible Chat Completions endpoint. You authenticate once with a CodeVibe API key and get access to models from Anthropic, OpenAI, Google, and more through a single base URL. No need to manage separate keys for each provider.

```
Your App  →  CodeVibe API (api.cline.bot compatibility endpoint)  →  Anthropic / OpenAI / Google / etc.
```
...
<CardGroup cols={2}>
  ...
  <Card title="Code Examples" icon="code" href="/api/sdk-examples">
    Ready-to-copy examples for Python, Node.js, curl, and the CodeVibe CLI.
  </Card>
</CardGroup>
...
Get your API key from the CodeVibe account console at [app.cline.bot](https://app.cline.bot) (Settings > API Keys), then follow the [Getting Started](/api/getting-started) guide.
