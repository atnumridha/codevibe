...
description: "Create an API key and make your first request to the Codie API in under a minute."
---
...
- A Codie account at the [Codie account console](https://app.codie.dev)
- `curl` or any HTTP client (Python, Node.js, etc.)

## Create an API Key

<Steps>
  <Step title="Sign in to Codie">
    Go to the [Codie account console](https://app.codie.dev) and sign in with your account.
  </Step>
...
| `402 Payment Required` | Your account has insufficient credits. Add credits at [app.codie.dev](https://app.codie.dev) |
| Empty response | Make sure `messages` is a non-empty array with at least one user message |
| Connection timeout | Verify your network can reach `api.codie.dev`. Check proxy settings if on a corporate network |
...
<CardGroup cols={2}>
  ...
  <Card title="SDK Examples" icon="code" href="/api/sdk-examples">
    Use the API from Python, Node.js, or the Codie CLI.
  </Card>
</CardGroup>
