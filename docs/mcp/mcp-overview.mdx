...
title: "MCP"
sidebarTitle: "MCP"
description: "Add, configure, and use MCP servers in CodeVibe."
---

MCP (Model Context Protocol) lets CodeVibe use external tools and data sources through MCP servers.

## What MCP gives you

- Connect CodeVibe to external APIs and services
- Add custom tools beyond built-in CodeVibe tools
- Use either local servers or remote hosted servers

## Quick start

1. Open **MCP Servers** in CodeVibe
...
Use CodeVibe's MCP Marketplace for one-click install when available.
1. In the CodeVibe panel, click the MCP Servers icon (stacked server icon in the top toolbar).
...
- **CLI:** `~/.codevibe/mcp.json` by default; legacy `~/.cline/mcp.json` remains supported for compatibility
- **IDE extensions:**
  1. In the CodeVibe panel, click the **MCP Servers** icon (stacked server icon in the top toolbar).
...
MCP also works in CodeVibe CLI. Configure servers in CLI MCP settings and use the same server definitions.

You can also list servers non-interactively:

```bash
codevibe config mcp
codevibe config mcp --json
```
