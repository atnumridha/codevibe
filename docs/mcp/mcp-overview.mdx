...
title: "MCP"
sidebarTitle: "MCP"
description: "Add, configure, and use MCP servers in Codie."
---

MCP (Model Context Protocol) lets Codie use external tools and data sources through MCP servers.

## What MCP gives you

- Connect Codie to external APIs and services
- Add custom tools beyond built-in Codie tools
- Use either local servers or remote hosted servers

## Quick start

1. Open **MCP Servers** in Codie
...
Use Codie's MCP Marketplace for one-click install when available.
1. In the Codie panel, click the MCP Servers icon (stacked server icon in the top toolbar).
...
MCP also works in Codie CLI. Configure servers in CLI MCP settings and use the same server definitions.
