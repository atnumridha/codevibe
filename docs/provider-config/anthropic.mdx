...
description: "Configure Anthropic API keys or Claude Code subscription with CodeVibe."
---
...
3.  **Create a Key:** Click "Create Key". Give your key a descriptive name (e.g., "CodeVibe").
...
Use this path if you already have Claude Max/Pro and want to use subscription-based access in CodeVibe.
...
2. In CodeVibe settings, select **Claude Code** as provider.
...
When extended thinking is in CodeVibe, the model generates `thinking` content blocks that detail its internal reasoning. These insights are then incorporated into its final response.
CodeVibe users can leverage this by checking the `Enable Extended Thinking` box below the model selection menu after selecting a Claude Model from any provider.
