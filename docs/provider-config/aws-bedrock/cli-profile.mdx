...
description: "Configure AWS Bedrock to use AWS CLI profiles for authentication with Codie."
---

### Overview

Codie can use AWS credentials or AWS CLI profiles to access AWS Bedrock. This guide configures an AWS CLI profile so Codie authenticates through SSO/federated roles.
...
3. Continue configuring your profile using [the `aws configure sso` CLI wizard](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sso.html#cli-configure-sso-configure)
    ...
    - Note which profile name you attach to your AWS account, this is needed to configure Codie in the following steps

4. If you haven't already done so, install VSCode and the Codie extension. Consult the [Getting Started](/getting-started/installing-codevibe) page for guidance.

5. Open the Codie extension, then click on the settings button ⚙️ to select your model source.
    - From the Model Source dropdown, select AWS Bedrock
...
After saving the profile name and region, use **Verify** in Codie settings to confirm the extension can reach AWS Bedrock with the selected profile.
