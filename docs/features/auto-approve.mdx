...
description: "Control which actions Codie can take without asking for approval."
---

Auto Approve lets you decide which actions Codie can take without prompting you each time. It keeps you out of approval popups during routine work while letting you keep tight control over high-risk actions.

If you find yourself repeatedly clicking approve for the same safe operations, Auto Approve fixes that.

## How It Works

Auto Approve is evaluated per tool call. When Codie is about to read a file, edit a file, run a command, or use the browser, it checks your Auto Approve settings for that category.
...
- **Terminal commands**: Codie treats commands as either safe or requiring approval. "Execute safe commands" covers safe commands. "Execute all commands" extends to commands flagged as requiring approval.
- **Notifications**: If enabled, Codie sends OS-level notifications when approval is required, and when an auto-approved terminal command has been running for 30 seconds.
...
Codie does not use a fixed allowlist. The model marks each command with a `requires_approval` flag based on the command and arguments. These are examples, not guarantees.
...
YOLO mode is Auto Approve on steroids. Check the box and Codie auto-approves everything: file changes, terminal commands, browser actions, MCP tools, and mode transitions.

<Note>
**Warning: This is dangerous.** YOLO mode disables all safety checks. Codie executes whatever it decides without asking permission.
</Note>
...
**Trusted, repetitive tasks** where you've validated Codie's approach and want to eliminate approval overhead.

**Demonstration purposes** where you want to show Codie's capabilities without interruptions.
...
- **Monitor the output.** Codie still shows what it's doing. Watch the terminal and file changes.
- **Keep version control handy.** Git becomes your safety net.

### Enabling YOLO Mode

Navigate to Codie Settings, then Features, and check "YOLO Mode." No confirmation dialogs. Once enabled, Codie auto-approves all actions immediately. Uncheck to disable.
