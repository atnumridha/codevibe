...
When your conversation approaches the model's context window limit, Codie automatically summarizes it to free up space and keep working.

## How It Works

Codie monitors token usage during your conversation. When you're getting close to the limit, it:
...
- Codie remembers everything it's done
- You can work on much larger projects without interruption

<Tip>
Auto Compact works especially well for long-running tasks. Structured task lists can help maintain progress across summarizations so Codie can stay on track across multiple context windows.
</Tip>
...
<Note>
With other models, Codie falls back to standard rule-based context truncation, even if Auto Compact is enabled.
</Note>
