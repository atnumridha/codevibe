...
Codie works with VS Code's multi-root workspaces, letting you manage multiple project folders or repositories in a single window. Whether you're working with a monorepo or separate Git repositories, Codie can read files, write code, and run commands across all of them.

<Warning>
Multi-root workspaces have two limitations:
- **Codie rules** only work in the primary workspace folder
...
Codie can complete tasks that span multiple projects or repositories:
...
Codie supports both patterns, as well as hybrid setups where some folders are Git repositories and others are not. The key difference: with multiple repositories, each folder has its own `.git` directory and Codie tracks them independently.
...
When you open separate Git repositories in one workspace, Codie treats each as an independent project with its own version control.

### What Codie Tracks Per Repository

For each workspace folder, Codie detects:
...
This means Codie understands that your frontend and backend might be at different commits, on different branches, or even use different version control systems.

<Note>
While Codie detects VCS information for all workspace folders, certain features only use the **primary workspace** (the first folder): [Codie rules](/customization/rules), [skills](/customization/skills#triggering-skills-with-slash-commands), and [Git-related features](/core-workflows/working-with-files) like `@git` mentions.
</Note>
...
Codie understands natural references to your workspaces:
...
- Codie needs to resolve ambiguity
...
Each service has its own repository. Codie can update the proto definitions and regenerate clients across all services.
...
The types repository defines interfaces used by both client and server. When you update a type, Codie can fix both consumers.
...
Mix of repositories and plain folders. Codie adapts to each folder's configuration.
...
[Codie rules](/customization/rules) (`.codevibe/rules/` directory, with legacy `.clinerules/` support) only work in the **primary workspace** (the first folder in your workspace). Rules in other workspace folders are ignored.
...
[Checkpoints](/core-workflows/checkpoints) are disabled in multi-root workspace mode. Codie displays a warning when this happens.
...
4. **Consider the primary workspace** for Codie rules placement
...
- Use [Plan mode](/core-workflows/plan-and-act) to let Codie understand structure first
- Add a `.codevibeignore` file to reduce noise, speed up scanning, and keep Codie focused on source code:
