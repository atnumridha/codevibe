...
CodeVibe works with VS Code's multi-root workspaces, letting you manage multiple project folders or repositories in a single window. Whether you're working with a monorepo or separate Git repositories, CodeVibe can read files, write code, and run commands across all of them.

<Warning>
Multi-root workspaces have two limitations:
- **CodeVibe rules** only work in the primary workspace folder
...
CodeVibe can complete tasks that span multiple projects or repositories:
...
CodeVibe supports both patterns, as well as hybrid setups where some folders are Git repositories and others are not. The key difference: with multiple repositories, each folder has its own `.git` directory and CodeVibe tracks them independently.
...
When you open separate Git repositories in one workspace, CodeVibe treats each as an independent project with its own version control.

### What CodeVibe Tracks Per Repository

For each workspace folder, CodeVibe detects:
...
This means CodeVibe understands that your frontend and backend might be at different commits, on different branches, or even use different version control systems.

<Note>
While CodeVibe detects VCS information for all workspace folders, certain features only use the **primary workspace** (the first folder): [CodeVibe rules](/customization/cline-rules), [skills](/customization/skills#triggering-skills-with-slash-commands), and [Git-related features](/core-workflows/working-with-files) like `@git` mentions.
</Note>
...
CodeVibe understands natural references to your workspaces:
...
- CodeVibe needs to resolve ambiguity
...
Each service has its own repository. CodeVibe can update the proto definitions and regenerate clients across all services.
...
The types repository defines interfaces used by both client and server. When you update a type, CodeVibe can fix both consumers.
...
Mix of repositories and plain folders. CodeVibe adapts to each folder's configuration.
...
[CodeVibe rules](/customization/cline-rules) (`.codevibe/rules/` directory, with legacy `.clinerules/` support) only work in the **primary workspace** (the first folder in your workspace). Rules in other workspace folders are ignored.

**Workaround:** Place shared rules in the primary workspace, or use global rules (`~/Documents/CodeVibe/Rules/` or `~/.codevibe/rules/`) which apply everywhere.

### Checkpoints

[Checkpoints](/core-workflows/checkpoints) are disabled in multi-root workspace mode. CodeVibe displays a warning when this happens.
...
4. **Consider the primary workspace** for CodeVibe rules placement
...
- Use [Plan mode](/core-workflows/plan-and-act) to let CodeVibe understand structure first
- Add a `.codevibeignore` file to reduce noise, speed up scanning, and keep CodeVibe focused on source code:
...
For more patterns and gotchas, see the [ignore file guide](/customization/clineignore).
