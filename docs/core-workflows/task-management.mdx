...
Every interaction with CodeVibe happens within a task. Tasks are self-contained work sessions that capture your entire conversation, code changes, command executions, and decisions.

## What are Tasks?

A task begins when you submit a prompt to CodeVibe. Your prompt defines the goal, and CodeVibe works toward it through conversation, code changes, and tool use. The quality of your initial prompt directly affects how well CodeVibe performs - clear, specific prompts lead to better results.
...
<Tip>
Want better results from CodeVibe? Start with a clear goal, concrete constraints, and the files or workflows you want the agent to inspect first.
</Tip>

## Scoping Your Tasks

Each task carries its own context: the conversation history, decisions made, and understanding built up over the session. How you scope your tasks directly affects how well CodeVibe can help you.
...
Every AI model has a context window - a limit on how much information it can process at once. Think of it as CodeVibe's working memory for the current task.

As you work, the context window fills up with:
- Your prompts and CodeVibe's responses
- File contents CodeVibe reads or edits
- Command outputs and tool results
- System instructions that guide CodeVibe's behavior (including [CodeVibe Rules](/customization/rules))

When the context window approaches its limit, CodeVibe automatically compresses older parts of the conversation to make room. This means very long tasks may lose some earlier details, though CodeVibe preserves the most important context.
...
If your starting context seems high even for simple prompts, add a [`.codevibeignore`](/customization/ignore-files) file to exclude dependencies, build artifacts, and other files CodeVibe doesn't need. This can dramatically reduce your baseline token usage.
...
Knowing when to start fresh versus continue can feel unclear at first. As you work with CodeVibe more, you'll develop an intuition for it. Use this table as a starting point:

| Scenario | Action | Why |
|----------|--------|-----|
| Switching to a different feature | **New task** | Clean context, focused responses |
| Building on work CodeVibe just completed | **Continue** | Shared understanding preserved |
| CodeVibe keeps going off-track | **New task** | Fighting context wastes time |
...
| Refining CodeVibe's last output | **Continue** | Momentum and decisions preserved |

To start a new task, click the **+** button in the CodeVibe sidebar or use the `/newtask` command. Your file changes are preserved through [checkpoints](/core-workflows/checkpoints), and you can reference previous tasks from history anytime.

## Understanding Task Costs

Every cloud-based AI model charges for usage based on tokens, the units of text the model processes. CodeVibe tracks these costs automatically and displays them in the task header so you can monitor spending as you work.

### How Costs Are Calculated

When you interact with CodeVibe, the model processes:
...
Cloud providers charge per million tokens, with output tokens typically costing more than input. Some providers also support **prompt caching**, which reduces costs when the same context (like your CodeVibe rules or large files) appears in multiple requests. CodeVibe automatically tracks cache savings when available.
...
|--------------|---------------|
| **CodeVibe provider** | Pay-per-use with credits you purchase |
...
Not ready to pay? CodeVibe offers several free paths:

- **Free models**: Search "free" in the model selector when using the CodeVibe provider. These models display a **FREE** tag and work well for learning and experimentation.
...
For most users, starting with free cloud models and moving to paid options as needed provides the best balance of cost, speed, and capability. Check [Selecting Your Model](/getting-started/authorization) for guidance on choosing the right option for your workflow.
...
Click the **History** button in the CodeVibe sidebar (clock icon at the top-right) to open the history view. You'll see all your past tasks with their initial prompt, timestamp, and token usage. Each task card expands to show a preview of the conversation.

### Searching Tasks

Use the search bar at the top of the history view to find specific tasks. The fuzzy search looks across everything: your prompts, CodeVibe's responses, code snippets, and file names.
...
CodeVibe can resume interrupted tasks with full context:

1. Open the task from history
2. CodeVibe loads the complete conversation
...
This works across sessions. Even if you close the editor and return days later, CodeVibe can pick up where you left off.
