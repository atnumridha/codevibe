...
---
<Warning>
  This feature currently only applies to CodeVibe CLI.
</Warning>
...
Run `codevibe connect` to open an interactive wizard that guides you through platform selection, credential entry, security configuration, and advanced options (provider, model, system prompt, agent mode).
...
|----------|---------------|---------------------|
| Telegram | `codevibe connect telegram` | Bot token |
| Slack | `codevibe connect slack` | Bot token plus webhook signing secret/base URL or socket app token |
| Discord | `codevibe connect discord` | Application ID, bot token, public key, base URL |
| Google Chat | `codevibe connect gchat` | Service account credentials JSON, base URL |
| WhatsApp | `codevibe connect whatsapp` | Phone number ID, access token, app secret, verify token, base URL |
| Linear | `codevibe connect linear` | API key, webhook signing secret, base URL |
...
<Steps>
  <Step title="Create a Telegram bot">
    ...
    1. Enter a display name (e.g., "CodeVibe")
    2. Enter a username ending in `bot` (e.g., `codevibe_myname_bot`). Must be unique across Telegram.
    3. BotFather responds with your bot token (looks like `7123456789:AAH...`)
  </Step>

  <Step title="Start the connector">
    ```bash
    codevibe connect telegram -k <BOT-TOKEN>
    ```
...
By default, anyone who finds your bot can message it and it will execute tasks on your machine. The `codevibe connect` wizard asks whether to restrict Telegram access and can configure this for you.
...
Use `--hook-command` only when you need custom access logic. The hook receives each incoming message with sender info via stdin. Your script returns `{"action": "allow"}` or `{"action": "deny", "message": "reason"}`. Without `--allowed-user-id` or `--hook-command`, everything is auto-approved, so restrict Telegram bots that can reach a running CodeVibe instance.
...
```bash
codevibe connect gchat --credentials <JSON> --base-url <URL>
```
...
```bash
codevibe connect whatsapp --phone-id <ID> --token <TOKEN> --app-secret <SECRET> --base-url <URL>
```
...
```bash
codevibe connect linear --api-key <KEY> --signing-secret <SECRET> --base-url <URL>
```
...
codevibe connect telegram --stop
```
...
```bash
# Terminal 1
codevibe connect telegram -k $TELEGRAM_TOKEN

# Terminal 2
codevibe connect slack --bot-token $SLACK_TOKEN --signing-secret $SECRET --base-url $URL
```

Connectors require the hub. Start it with `codevibe hub start` if it doesn't auto-start.
