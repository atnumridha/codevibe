...
---
<Warning>
  This feature currently only applies to Codie CLI.
</Warning>
...
<Steps>
  <Step title="Create a Telegram bot">
    ...
    1. Enter a display name (e.g., "Codie")
...
Use `--hook-command` only when you need custom access logic. The hook receives each incoming message with sender info via stdin. Your script returns `{"action": "allow"}` or `{"action": "deny", "message": "reason"}`. Without `--allowed-user-id` or `--hook-command`, everything is auto-approved, so restrict Telegram bots that can reach a running Codie instance.
