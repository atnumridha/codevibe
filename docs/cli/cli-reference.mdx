---
title: "CLI Reference"
description: "Complete command reference for Codie CLI including all commands, flags, and configuration options."
---
...
Codie CLI - editor-native coding agent in your terminal
...
Manage Codie plugins. Install plugins from file URLs, npm, git repositories, or local paths. See [Plugins](/customization/plugins) for full details and the plugin manifest format.
...
Legacy `CLINE_*` environment variables remain accepted as compatibility aliases so existing automation keeps working while Codie tracks upstream changes.
...
Codie also reads selected `.cline/` and `~/.cline/` paths for upstream compatibility and migration.
