...
```bash
codevibe --team-name auth-sprint "Plan and implement user authentication with tests"
```
...
```bash
codevibe --team-name auth-sprint "Continue with incomplete tasks"
```
...
Team state is stored at `~/.codevibe/data/teams/[team-name]/` by default. Existing `~/.cline/data/teams/[team-name]/` state is still recognized for legacy compatibility. Team state includes:
