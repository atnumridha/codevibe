...
description: "Automated GitHub issue analysis using CodeVibe CLI to identify root causes."
---

Automated GitHub issue analysis using CodeVibe CLI. This script uses CodeVibe's autonomous AI capabilities to fetch, analyze, and identify root causes of GitHub issues, outputting clean, parseable results that can be easily integrated into your development workflows.

<Note>
**New to CodeVibe CLI?** This sample assumes you have already completed the [Installation Guide](/getting-started/installing-codevibe) and authenticated with `codevibe auth`. If you haven't set up CodeVibe CLI yet, please start there first.
</Note>
...
- **CodeVibe CLI** installed and authenticated ([Installation Guide](/getting-started/installing-codevibe))
- **At least one AI model provider** configured (e.g., OpenRouter, Anthropic, OpenAI)
- **Basic familiarity** with CodeVibe CLI commands
...
<Note>
The script will automatically handle everything: fetching the issue, analyzing it with CodeVibe, and displaying the results. The analysis typically takes 30-60 seconds depending on the issue complexity.
</Note>
...
**1. `codevibe --auto-approve true --json "$PROMPT: $ISSUE_URL"`**
   - `codevibe` is the CodeVibe CLI binary
...
This sample demonstrates how to build an autonomous GitHub issue analysis tool using CodeVibe CLI:

1. **Building autonomous CLI tools** using CodeVibe's capabilities
2. **Parsing structured JSON output** from CodeVibe CLI
