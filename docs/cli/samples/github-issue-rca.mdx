...
description: "Automated GitHub issue analysis using Codie CLI to identify root causes."
---

Automated GitHub issue analysis using Codie CLI. This script uses Codie's autonomous AI capabilities to fetch, analyze, and identify root causes of GitHub issues, outputting clean, parseable results that can be easily integrated into your development workflows.

<Note>
**New to Codie CLI?** This sample assumes you have already completed the [Installation Guide](/getting-started/installing-codevibe) and authenticated with `codevibe auth`. If you haven't set up Codie CLI yet, please start there first.
</Note>
...
- **Codie CLI** installed and authenticated ([Installation Guide](/getting-started/installing-codevibe))
- **At least one AI model provider** configured (e.g., OpenRouter, Anthropic, OpenAI)
- **Basic familiarity** with Codie CLI commands
...
```bash
curl -O https://raw.githubusercontent.com/atnumridha/codevibe/main/apps/vscode/src/samples/cli/github-issue-rca/analyze-issue.sh
```
...
<Note>
The script will automatically handle everything: fetching the issue, analyzing it with Codie, and displaying the results. The analysis typically takes 30-60 seconds depending on the issue complexity.
</Note>
...
**1. `codevibe --auto-approve true --json "$PROMPT: $ISSUE_URL"`**
   - `codevibe` is the Codie CLI binary
...
This sample demonstrates how to build an autonomous GitHub issue analysis tool using Codie CLI:

1. **Building autonomous CLI tools** using Codie's capabilities
2. **Parsing structured JSON output** from Codie CLI
...
- [CLI Installation Guide](/getting-started/installing-codevibe)
