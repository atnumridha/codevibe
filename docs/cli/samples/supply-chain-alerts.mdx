...
description: "Schedule the CodeVibe CLI to scan your machine for compromised packages with Bumblebee and text you on Telegram when it finds one."
---
...
- The CodeVibe CLI scheduler, which runs an agent on a cron schedule.
- The CodeVibe CLI Telegram connector, which delivers the result to a chat.

The end result: every morning, a CodeVibe agent pulls the latest threat intelligence, runs a read-only scan, and texts you a green check if you are clean or a red alert with details if you are exposed.

```mermaid
flowchart TD
    cron["codevibe schedule (daily)"] --> agent["CodeVibe agent"]
...
- Node.js 22 or newer (for the CodeVibe CLI).
- Go 1.22 or newer (to build Bumblebee).
- A Telegram account.
- An AI provider key, or a CodeVibe account.

## 1. Install the CodeVibe CLI

```bash
npm install -g codevibe
codevibe # run once to configure inference provider and model
```
...
<Steps>
  ...
  <Step title="Start the connector">
    ...
    ```bash
    codevibe connect telegram -k "<BOT-TOKEN>" --cwd ~/tools/bumblebee
    ```
...
<Warning>
By default, anyone who finds your bot can message it and it will run tasks on your machine. Lock it down before leaving the connector running. The `codevibe connect` wizard can guide you through Telegram user ID setup, or you can message [@userinfobot](https://t.me/userinfobot) and restart the connector with your allowed user ID:
...
Create the schedule with the CodeVibe CLI on the same machine and pass the delivery method explicitly. The running Telegram connector delivers the result to its chat:

```bash
codevibe schedule create "supply-chain-watch" \
  --cron "0 8 * * *" \
...
- The connector process (`codevibe connect telegram`) must stay running for delivery to work. Run it under a process manager (systemd, launchd, `pm2`, or a `tmux`/`screen` session) so it survives reboots.
- The hub runs the schedule and starts automatically when you create one. If it is not running, start it with `codevibe hub start`.
- Manage schedules anytime with `codevibe schedule list`, `codevibe schedule pause <id>`, `codevibe schedule resume <id>`, and `codevibe schedule delete <id>`.
