...
description: "Schedule the Codie CLI to scan your machine for compromised packages with Bumblebee and text you on Telegram when it finds one."
---
...
- The Codie CLI scheduler, which runs an agent on a cron schedule.
- The Codie CLI Telegram connector, which delivers the result to a chat.

The end result: every morning, a Codie agent pulls the latest threat intelligence, runs a read-only scan, and texts you a green check if you are clean or a red alert with details if you are exposed.

```mermaid
flowchart TD
    cron["codevibe schedule (daily)"] --> agent["Codie agent"]
...
- Node.js 22 or newer (for the Codie CLI).
- Go 1.22 or newer (to build Bumblebee).
- A Telegram account.
- An AI provider key, or a Codie account.

## 1. Install the Codie CLI
...
Create the schedule with the Codie CLI on the same machine and pass the delivery method explicitly. The running Telegram connector delivers the result to its chat:
