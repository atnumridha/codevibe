...
Codie CLI's `--config` and `--thinking` flags enable sophisticated multi-model workflows. Instead of using a single model for all tasks, you can route different work to different models based on cost, capability, and specialization.
...
See our production GitHub Actions workflow that uses Codie CLI for automated PR reviews: [codevibe-pr-review.yml](https://github.com/atnumridha/codevibe/blob/main/.github/workflows/codevibe-pr-review.yml)
...
<Note>
Parallel execution requires managing multiple Codie instances. See [Multi-instance workflows](/usage/cli-overview#automation-patterns) for details.
</Note>

## Extended Thinking for Complex Tasks

Use the `--thinking` flag when Codie needs to analyze multiple approaches:
...
<Columns cols={2}>
  ...
  <Card title="Codie provider" icon="brain" href="/getting-started/codevibe-provider">
  ...
  <Card title="CI/CD Integration" icon="github" href="/cli/samples/github-integration">
    Automate GitHub workflows with Codie CLI
  </Card>
</Columns>
