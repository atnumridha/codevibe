...
CodeVibe CLI's `--config` and `--thinking` flags enable sophisticated multi-model workflows. Instead of using a single model for all tasks, you can route different work to different models based on cost, capability, and specialization.
...
See our production GitHub Actions workflow that uses CodeVibe CLI for automated PR reviews: [codevibe-pr-review.yml](https://github.com/atnumridha/codevibe/blob/main/.github/workflows/codevibe-pr-review.yml)
...
SUMMARY=$(echo "$ISSUE_CONTENT" | codevibe --auto-approve true --config ~/.codevibe-haiku \
  "summarize this issue in 2-3 sentences")

# Phase 2: Detailed plan with expensive model + thinking
PLAN=$(echo "$SUMMARY" | codevibe --auto-approve true --thinking high --config ~/.codevibe-opus \
  "create detailed implementation plan with edge cases")

# Phase 3: Execute with mid-tier model
echo "$PLAN" | codevibe --auto-approve true --config ~/.codevibe-sonnet \
  "implement the plan from above"
```

<Note>
Each `codevibe` invocation needs to complete before passing output to the next phase. Use shell variables to store intermediate results rather than piping `codevibe` commands directly.
</Note>
...
mkdir -p ~/.codevibe-haiku ~/.codevibe-sonnet ~/.codevibe-opus

# Configure each with different models
codevibe --config ~/.codevibe-haiku auth anthropic --modelid claude-haiku-4-20250514
codevibe --config ~/.codevibe-sonnet auth anthropic --modelid claude-sonnet-4-20250514
codevibe --config ~/.codevibe-opus auth anthropic --modelid claude-opus-4-5-20251101

# Or use different providers entirely
codevibe --config ~/.codevibe-gemini auth gemini --modelid gemini-2.0-flash-exp
codevibe --config ~/.codevibe-codex auth openai-codex --modelid gpt-5-latest
```

Now you can switch models per-task with `--config`:

```bash
codevibe --config ~/.codevibe-haiku "quick task"
codevibe --config ~/.codevibe-opus "complex reasoning task"
```
...
git show | codevibe --auto-approve true --config ~/.codevibe-gemini "review and save to gemini-review.md" &
git show | codevibe --auto-approve true --config ~/.codevibe-codex "review and save to codex-review.md" &
git show | codevibe --auto-approve true --config ~/.codevibe-opus "review and save to opus-review.md" &

# Wait for all to complete
wait

# Synthesize
cat *-review.md | codevibe --auto-approve true "create consensus review"
```

<Note>
Parallel execution requires managing multiple CodeVibe instances. See [Multi-instance workflows](/usage/cli-overview#automation-patterns) for details.
</Note>

## Extended Thinking for Complex Tasks

Use the `--thinking` flag when CodeVibe needs to analyze multiple approaches:

```bash
# Without thinking: Fast but may miss nuances
codevibe --auto-approve true "refactor this codebase"

# With thinking: Slower but more thorough
codevibe --auto-approve true --thinking high \
...
gh pr view $PR | codevibe --auto-approve true --config ~/.codevibe-haiku \
    "list all issues to fix, output as JSON"

# Opus with thinking: Deep analysis only if issues found
if [ -s issues.json ]; then
    codevibe --auto-approve true --thinking high --config ~/.codevibe-opus \
...
git diff main | codevibe --auto-approve true --config ~/.codevibe-gemini "security review" > gemini-sec.md &
git diff main | codevibe --auto-approve true --config ~/.codevibe-opus "security review" > opus-sec.md &
git diff main | codevibe --auto-approve true --config ~/.codevibe-codex "security review" > codex-sec.md &
wait

# High-priority: Issues all 3 models found
cat *-sec.md | codevibe --auto-approve true "find security issues all 3 reviews mentioned"
```
...
<Columns cols={2}>
  ...
  <Card title="CodeVibe provider" icon="brain" href="/getting-started/codevibe-provider">
  ...
  <Card title="CI/CD Integration" icon="github" href="/cli/samples/github-integration">
    Automate GitHub workflows with CodeVibe CLI
  </Card>
</Columns>
