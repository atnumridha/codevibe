...
description: "Configure Single Sign-On (SSO) for Codie Enterprise via WorkOS AuthKit."
---

## Overview
Codie Enterprise integrates with your identity provider (IdP) via **WorkOS AuthKit** for SSO.

This page describes, at a high level, how SSO is set up for Codie Enterprise using WorkOS AuthKit.
...
## Using the Codie Dashboard
Use the Codie Dashboard at https://app.codie.dev to:
...
To connect your IdP to Codie Enterprise, configure your identity provider in **WorkOS AuthKit**:
...
Codie Enterprise’s default SSO integration is **via WorkOS**.
...
1) Attempt an SSO sign-in from https://app.codie.dev.
2) Confirm the sign-in completes (you are redirected back successfully).

## Troubleshooting
- **Redirect URI mismatch**: confirm the redirect/callback URL configured in WorkOS matches what was provided during your Codie Enterprise onboarding.
