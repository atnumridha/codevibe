...
description: "Complete reference of OpenTelemetry log events emitted by CodeVibe"
---

This page documents all OpenTelemetry log events currently instrumented in CodeVibe. These events are emitted when OpenTelemetry integration is enabled and provide detailed insights into user behavior, task execution, and system operations.
...
CodeVibe emits events across several categories, each prefixed with a namespace:
...
| `task.rule_toggled` | CodeVibe rule toggled on/off | rule_name, enabled, is_global |
| `task.yolo_mode_toggled` | YOLO mode toggled | enabled |
| `task.cline_web_tools_toggled` | CodeVibe web tools setting toggled | enabled |
...
| `extension_version` | string | CodeVibe extension version |
...
sequenceDiagram
    ...
    User->>CodeVibe: Start Task
    CodeVibe->>OTel: task.created
    
    User->>CodeVibe: Submit Message
    CodeVibe->>OTel: task.conversation_turn (user)
    
    CodeVibe->>CodeVibe: Process with AI
    CodeVibe->>OTel: task.tokens
    CodeVibe->>OTel: task.conversation_turn (assistant)
    
    CodeVibe->>CodeVibe: Use Tool
    CodeVibe->>OTel: task.tool_used
    
    User->>CodeVibe: Provide Feedback
    CodeVibe->>OTel: task.option_selected
    
    User->>CodeVibe: Complete Task
    CodeVibe->>OTel: task.completed
```
