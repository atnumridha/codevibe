...
description: "Complete reference of OpenTelemetry log events emitted by Codie"
---

This page documents all OpenTelemetry log events currently instrumented in Codie. These events are emitted when OpenTelemetry integration is enabled and provide detailed insights into user behavior, task execution, and system operations.
...
Codie emits events across several categories, each prefixed with a namespace:
...
| `task.rule_toggled` | Codie rule toggled on/off | rule_name, enabled, is_global |
| `task.yolo_mode_toggled` | YOLO mode toggled | enabled |
| `task.cline_web_tools_toggled` | Codie web tools setting toggled | enabled |
...
| `extension_version` | string | Codie extension version |
...
sequenceDiagram
    ...
    User->>Codie: Start Task
    Codie->>OTel: task.created
    
    User->>Codie: Submit Message
    Codie->>OTel: task.conversation_turn (user)
    
    Codie->>Codie: Process with AI
    Codie->>OTel: task.tokens
    Codie->>OTel: task.conversation_turn (assistant)
    
    Codie->>Codie: Use Tool
    Codie->>OTel: task.tool_used
    
    User->>Codie: Provide Feedback
    Codie->>OTel: task.option_selected
    
    User->>Codie: Complete Task
    Codie->>OTel: task.completed
```
