...
description: "Optional telemetry and observability for your CodeVibe deployment"
---

CodeVibe includes optional monitoring capabilities for organizations that want to track usage and integrate with their observability infrastructure.

## Monitoring Options

<CardGroup cols={2}> 
  <Card title="CodeVibe Telemetry" icon="chart-simple" href="/enterprise-solutions/monitoring/telemetry">
    Built-in anonymous usage tracking that helps improve CodeVibe (opt-in)
  </Card>
...
CodeVibe includes opt-in telemetry for anonymous usage tracking:
...
Users can enable or disable telemetry in CodeVibe settings. All data is anonymous and does not include code content, file paths, or sensitive information.

See [CodeVibe Telemetry](/enterprise-solutions/monitoring/telemetry) for configuration details.

## OpenTelemetry Integration

For advanced monitoring needs, CodeVibe supports OpenTelemetry's OTLP (OpenTelemetry Protocol) for exporting metrics and logs to your own infrastructure.
...
### When to Use CodeVibe Telemetry
- You want to help improve CodeVibe through anonymous usage data
...
For basic telemetry, enable it in CodeVibe settings. For OpenTelemetry, see the configuration guide.
</Step>
...
All CodeVibe monitoring features are designed with privacy in mind:
