...
Prompt Storage allows enterprises to automatically back up Codie conversation history to cloud storage (AWS S3 or Cloudflare R2). This provides a centralized repository for compliance, audit trails, and usage analysis while maintaining local storage as the primary source of truth.

## Overview

Every Codie task conversation is stored locally in `~/.codevibe/data/tasks/<taskId>/api_conversation_history.json` by default. Existing legacy task history can still be imported during migration. When prompt storage is enabled, a background sync worker automatically uploads these conversation files to your configured S3 or R2 bucket.
...
```mermaid
graph LR
    A[User] --> B[Codie Extension]
    B --> C[Local Storage<br/>~/.codevibe/data/tasks/]
...
- ❌ Workspace files not accessed by Codie
- ❌ API keys or secrets
  ...
  <Tab title="AWS S3">
    ...
    <Steps>
      <Step title="Create S3 Bucket">
        Create a dedicated S3 bucket for Codie conversation storage:

        ```bash
        aws s3 mb s3://your-codevibe-prompts --region us-east-1
        ```
      ...
      <Step title="Create IAM Policy">
          ...
          "Statement": [
            {
              ...
              "Resource": "arn:aws:s3:::your-codevibe-prompts/*"
            },
            {
              "Effect": "Allow",
              "Action": [
        ...
        Save this as `codevibe-prompt-storage-policy.json` and create the policy:

        ```bash
        aws iam create-policy \
          --policy-name CodiePromptStorage \
          --policy-document file://codevibe-prompt-storage-policy.json
        ```
      </Step>
      ...
      <Step title="Create IAM User">
        ...
        ```bash
        aws iam create-user --user-name codevibe-prompt-uploader

        aws iam attach-user-policy \
          --user-name codevibe-prompt-uploader \
          --policy-arn arn:aws:iam::YOUR_ACCOUNT_ID:policy/CodiePromptStorage

        aws iam create-access-key --user-name codevibe-prompt-uploader
        ```

        Save the `AccessKeyId` and `SecretAccessKey` from the output.
      </Step>

      <Step title="Configure in Codie Dashboard">
        In the Codie admin console at [app.codevibe.bot](https://app.codevibe.bot):
  ...
  <Tab title="Cloudflare R2">
    ...
    <Steps>
      <Step title="Create R2 Bucket">
        ...
        4. Name your bucket (e.g., `codevibe-prompts`)
      ...
      <Step title="Generate API Token">
        ...
        3. Configure permissions:
           - **Token name**: Codie Prompt Storage
...
- On first startup, scans all existing tasks in `~/.codevibe/data/tasks/` and any imported legacy task history
- Queues conversation files that haven't been uploaded
- Useful for enabling prompt storage on an existing Codie deployment
...
While prompt storage operates independently, it integrates with Codie's observability system:
...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=BucketName,Value=your-codevibe-prompts \
  ...
  "Statement": [
    {
      ...
      "Action": "s3:*",
      "Resource": [
        "arn:aws:s3:::your-codevibe-prompts/*",
        "arn:aws:s3:::your-codevibe-prompts"
      ],
      "Condition": {
        "Bool": {
...
aws s3api put-bucket-logging \
  ...
  --bucket-logging-status '{
    "LoggingEnabled": {
      ...
      "TargetPrefix": "codevibe-prompts-access/"
    }
  }'
```
...
2. Look for `[CodieBlobStorage]` and `[SyncWorker]` log entries
...
<CardGroup cols={3}>
  ...
  <Card title="Telemetry" icon="chart-simple" href="/enterprise-solutions/monitoring/telemetry">
    Learn about Codie's built-in anonymous usage tracking
  </Card>
