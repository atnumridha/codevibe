...
description: "Configure CodeVibe to work behind firewalls and proxies"
---

If you're working behind a corporate proxy or firewall, you'll need to configure
proxy settings for CodeVibe to connect to AI providers. The configuration varies
depending on which version of CodeVibe you're using.
...
is needed for CodeVibe itself.

## CLI

The CodeVibe CLI uses standard HTTP proxy environment variables. Configure these before running `codevibe` commands.
...
CodeVibe CLI only supports HTTP proxies. It does not support SOCKS proxies,
...
After configuring the proxy, test that CodeVibe can connect to your AI provider:

1. Open the CodeVibe panel
...
CodeVibe in JetBrains only supports HTTP proxies. It does not support SOCKS
proxies, proxy autoconfiguration (PAC) scripts, or HTTP proxies which require
authentication beyond a basic username and password.

CodeVibe does not pick up changed proxy settings dynamically. After changing proxy
settings, restart the IDE for CodeVibe to use the new settings.
...
3. Use curl to verify the certificate works, for example, `curl -x proxy.corp.example:8080 --cacert /path/to/ca-cert.pem -o - -vv https://api.codevibe.bot/`
4. Consider disabling `http.proxyStrictSSL` in VSCode (not recommended for production)

### Testing Proxy Configuration

If you encounter problems with CodeVibe networking, first verify your proxy
...
Next, check ~/.codevibe/codevibe-core-service.log (CLI, JetBrains) for log messages
