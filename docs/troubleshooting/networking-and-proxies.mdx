...
description: "Configure Codie to work behind firewalls and proxies"
---

If you're working behind a corporate proxy or firewall, you'll need to configure
proxy settings for Codie to connect to AI providers. The configuration varies
depending on which version of Codie you're using.
...
is needed for Codie itself.

## CLI

The Codie CLI uses standard HTTP proxy environment variables. Configure these before running `codevibe` commands.
...
Codie CLI only supports HTTP proxies. It does not support SOCKS proxies,
...
After configuring the proxy, test that Codie can connect to your AI provider:

1. Open the Codie panel
...
Codie in JetBrains only supports HTTP proxies. It does not support SOCKS
proxies, proxy autoconfiguration (PAC) scripts, or HTTP proxies which require
authentication beyond a basic username and password.

Codie does not pick up changed proxy settings dynamically. After changing proxy
settings, restart the IDE for Codie to use the new settings.
...
If you encounter problems with Codie networking, first verify your proxy
