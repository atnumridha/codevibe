...
export const CODEVIBE_AGENT_DEFAULT_MODEL_ID = "gpt-5.5"
...
export function getProviderDisplayLabel(providerId: string): string {
	...
	return CODEVIBE_PROVIDER_LABELS[normalizedProvider] ?? titleCaseProviderId(normalizedProvider)
}
...
export function prioritizeCodeVibeProviderIds(providerIds: string[]): string[] {
	...
	return normalized.sort((a, b) => {
		...
		return getProviderDisplayLabel(a).localeCompare(getProviderDisplayLabel(b)) || a.localeCompare(b)
	})
}

export function prioritizeCodeVibeProviders<T extends { id: string }>(providers: T[]): T[] {
	const orderedIds = prioritizeCodeVibeProviderIds(providers.map((provider) => provider.id))
	const orderById = new Map(orderedIds.map((providerId, index) => [providerId, index]))
	...
	return [...providers].sort((a, b) => {
		...
		return getProviderDisplayLabel(aId).localeCompare(getProviderDisplayLabel(bId)) || aId.localeCompare(bId)
	})
}
