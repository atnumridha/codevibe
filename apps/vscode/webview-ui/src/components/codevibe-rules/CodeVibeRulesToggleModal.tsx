...
const CodeVibeRulesToggleModal: React.FC = () => {
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleClineRule(
			...
			.catch((error) => {
				console.error("Error toggling guidance rule:", error)
			})
	}
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Codie Guidance</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Codie Guidance" : "Show Codie Guidance"}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Codie with system-level guidance. Think of them as a persistent way
								...
								<p>
									Workflows allow you to define a series of steps to guide Codie through a repetitive set of
								...
								<p>
									Skills are reusable instruction sets that Codie can activate on-demand. When a task matches a
									skill's description, Codie uses the <span className="font-bold">use_skill</span> tool to load
									the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in Codie's execution lifecycle,
							...
							<>
								...
								{isWindows && (
									<div className="flex items-center gap-2 px-3 py-3 mb-4 bg-vscode-inputValidation-warningBackground border-l-[3px] border-vscode-inputValidation-warningBorder">
										...
										<span className="text-base">
											Hook toggling is not yet supported on Windows. Hooks can be created, edited, and
											deleted, and execute whenever the hook file exists.
										</span>
									</div>
								)}
								...
								{workspaceHooks.map((workspace, index) => (
									<div
										...
										<div className="text-sm font-normal mb-2">Workspace hooks for {workspace.workspaceName}</div>
										<div className="flex flex-col gap-0">
											{workspace.hooks
												.sort((a, b) => a.name.localeCompare(b.name))
												.map((hook) => (
...
export default CodeVibeRulesToggleModal
