...
import { CLINE_RECOMMENDED_MODELS_FALLBACK as CODEVIBE_RECOMMENDED_MODELS_FALLBACK } from "@shared/cline/recommended-models"
...
export interface CodeVibeModelPickerProps {
	isPopup?: boolean
	currentMode: Mode
...
const CODEVIBE_RECOMMENDED_MODELS_RETRY_DELAY_MS = 5000
...
const RECOMMENDED_MODELS_FALLBACK: FeaturedModelCardEntry[] = CODEVIBE_RECOMMENDED_MODELS_FALLBACK.recommended
	.map((model) => toFeaturedModelCardEntry(model, "RECOMMENDED"))
	.filter((model): model is FeaturedModelCardEntry => model !== null)

const FREE_MODELS_FALLBACK: FeaturedModelCardEntry[] = CODEVIBE_RECOMMENDED_MODELS_FALLBACK.free
	.map((model) => toFeaturedModelCardEntry(model, "FREE"))
	.filter((model): model is FeaturedModelCardEntry => model !== null)

const CodeVibeModelPicker: React.FC<CodeVibeModelPickerProps> = ({
	isPopup,
	currentMode,
	showProviderRouting,
	initialTab,
	...
	const fetchClineRecommendedModels = useCallback(async () => {
		...
		if (clineRecommendedModelsRetryTimeoutRef.current === null) {
			...
			}, CODEVIBE_RECOMMENDED_MODELS_RETRY_DELAY_MS)
		}
			...
			<div style={{ display: "flex", flexDirection: "column" }}>
				...
				<DropdownWrapper ref={dropdownRef}>
					<VSCodeTextField
						...
						style={{
							width: "100%",
							zIndex: CODEVIBE_MODEL_PICKER_Z_INDEX,
							position: "relative",
						}}
						value={searchTerm}>
						{searchTerm && (
...
export default CodeVibeModelPicker
...
const CODEVIBE_MODEL_PICKER_Z_INDEX = 1_000
...
const DropdownList = styled.div`
	...
	z-index: ${CODEVIBE_MODEL_PICKER_Z_INDEX - 1};
