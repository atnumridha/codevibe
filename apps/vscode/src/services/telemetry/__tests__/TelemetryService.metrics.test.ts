...
function createTelemetryService(provider: FakeProvider, overrides: Partial<TelemetryMetadata> = {}): TelemetryService {
	return new TelemetryService([provider], {
		...
		codevibe_type: "codevibe-unit-tests",
...
describe("TelemetryService metrics", () => {
	...
	it("captureConversationTurnEvent emits counters with cache and cost", () => {
		...
		const costEntry = provider.counters.find((entry) => entry.name === "codevibe.cost.total")
		assert.ok(costEntry)
	...
	it("captureWorkspaceInitialized emits gauge and retires previous series", () => {
		...
		const initialSeries = provider.gauges.get("codevibe.workspace.active_roots")
		...
		const updatedSeries = provider.gauges.get("codevibe.workspace.active_roots")
	...
	it("captureGrpcResponseSize records histogram with correct name, value, and attributes", () => {
		...
		assert.strictEqual(entry.attributes.service, "codevibe.StateService")
