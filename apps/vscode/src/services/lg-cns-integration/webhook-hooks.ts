...
export async function writeLgWebhookConfig(webhookUrl: string, webhookToken: string): Promise<void> {
	...
	const clineDir = path.join(documentsPath, "Codie")
...
try {
    ...
    $configPath = Join-Path (Join-Path $documentsPath "Codie") "webhook_config.json"
...
async function readConfig() {
  const configPath = path.join(os.homedir(), "Documents", "Codie", "webhook_config.json")
  try {
