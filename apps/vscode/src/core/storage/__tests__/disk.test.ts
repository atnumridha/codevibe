...
describe("disk - hooks functionality", () => {
	...
	describe("getWorkspaceHooksDirs", () => {
		...
		it("should return CodeVibe hooks directory when it exists", async () => {
			// Create workspace root with hooks directory
			const workspaceRoot = path.join(tempDir, "workspace1")
			const hooksDir = path.join(workspaceRoot, ".codevibe", "hooks")
		...
		it("should return legacy hooks directory when only legacy path exists", async () => {
			const workspaceRoot = path.join(tempDir, "workspace1")
			...
			const result = await getWorkspaceHooksDirs()
			result.should.deepEqual([hooksDir])
		})

		it("should return CodeVibe hooks before legacy hooks when both exist", async () => {
			const workspaceRoot = path.join(tempDir, "workspace1")
			const codevibeHooksDir = path.join(workspaceRoot, ".codevibe", "hooks")
			const legacyHooksDir = path.join(workspaceRoot, ".clinerules", "hooks")
			await fs.mkdir(codevibeHooksDir, { recursive: true })
			...
			const result = await getWorkspaceHooksDirs()
			result.should.deepEqual([codevibeHooksDir, legacyHooksDir])
		})
		...
		it("should not return hooks directory if it's a file instead of directory", async () => {
			...
			const workspaceRoot = path.join(tempDir, "workspace1")
			const hooksPath = path.join(workspaceRoot, ".codevibe", "hooks")
		...
		it("should return multiple hooks directories for multi-root workspace", async () => {
			...
			const hooksDir1 = path.join(workspaceRoot1, ".codevibe", "hooks")
			const hooksDir2 = path.join(workspaceRoot2, ".codevibe", "hooks")
		...
		it("should return only existing hooks directories in multi-root workspace", async () => {
			...
			result.should.not.containEql(path.join(workspaceRoot2, ".codevibe", "hooks"))
		...
		it("should use correct path joining for hooks directory", async () => {
			const workspaceRoot = path.join(tempDir, "workspace1")
			const expectedHooksDir = path.join(workspaceRoot, ".codevibe", "hooks")
			...
			result[0].should.match(/\.codevibe[\\/]hooks$/)
		})
		...
		it("should expose CodeVibe and legacy hook candidates in stable order", () => {
			const workspaceRoot = path.join(tempDir, "workspace1")
			getWorkspaceHookDirCandidates(workspaceRoot).should.deepEqual([
				path.join(workspaceRoot, ".codevibe", "hooks"),
				path.join(workspaceRoot, ".clinerules", "hooks"),
			])
		})
	})
