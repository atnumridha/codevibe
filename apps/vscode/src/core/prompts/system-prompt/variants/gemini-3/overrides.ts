...
- Prepare a visual plan for non-trivial work: a fenced \`\`\`mermaid flowchart, sequenceDiagram, stateDiagram, or classDiagram that maps the proposed flow, ownership boundaries, or implementation sequence. If a diagram would add no value, say so briefly in the plan.
...
3. **Visual Plan** (Mermaid diagram or one-sentence rationale)
   Include a fenced \`\`\`mermaid diagram for multi-file, multi-component, stateful, or phased work. Use flowchart/sequence/state/class diagrams based on the problem shape.
