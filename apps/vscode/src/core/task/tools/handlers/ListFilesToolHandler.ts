...
export class ListFilesToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		// Check workspace ignore access before performing any IO.
		...
		if (!accessValidation.ok) {
			config.taskState.consecutiveMistakeCount++
			if (!config.isSubagentExecution) {
				await config.callbacks.say("workspace_ignore_error", relDirPath)
			}
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `CodeVibe wants to view directory ${getWorkspaceBasename(absolutePath, "ListFilesToolHandler.notification")}/`
