	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (block.name === "replace_in_file" && !rawDiff) {
			...
			await config.callbacks.say(
				"error",
				`Codie tried to use replace_in_file for '${relPath}' without value for required parameter 'diff'. Retrying...`,
			)
		...
		if (block.name === "write_to_file" && rawContent == null) {
			...
			await config.callbacks.say(
				"error",
				`Codie tried to use write_to_file for '${relPath}' without value for required parameter 'content'. ${
					config.taskState.consecutiveMistakeCount >= 2
						? "This has happened multiple times - Codie will try a different approach."
						: "Retrying..."
				}`,
			)
		...
		try {
			...
			if (await config.callbacks.shouldAutoApproveToolWithPath(block.name, relPath)) {
				...
				const notificationMessage = `Codie wants to ${fileExists ? "edit" : "create"} ${getWorkspaceBasename(relPath, "WriteToFile.notification")}`
