...
describe("Hook System", () => {
	...
	describe("StdioHookRunner", () => {
		it("should execute workspace hook from its respective workspace root directory", async function () {
			...
			const hookPath = path.join(tempDir, ".codevibe", "hooks", "PreToolUse")
...
console.log(JSON.stringify({
  ...
  contextModification: "CWD: " + process.cwd()
		...
		it("should prefer CodeVibe workspace hook over legacy duplicate", async () => {
			const primaryHooksDir = path.join(tempDir, ".codevibe", "hooks")
			const legacyHooksDir = path.join(tempDir, ".clinerules", "hooks")
			await fs.mkdir(legacyHooksDir, { recursive: true })
			stubHookDirs(sandbox, [primaryHooksDir, legacyHooksDir])

			const primaryHookPath = path.join(primaryHooksDir, "PreToolUse")
			const legacyHookPath = path.join(legacyHooksDir, "PreToolUse")
...
console.log(JSON.stringify({
  ...
  contextModification: "LEGACY"
			...
			result.cancel.should.be.false()
			result.contextModification?.should.equal("PRIMARY")
		})
  ...
  contextModification: "Received tool: " + input.preToolUse.toolName
	...
	describe("PostToolUse Hook", () => {
		it("should receive execution results", async () => {
			const hookPath = path.join(tempDir, ".codevibe", "hooks", "PostToolUse")
			const hookScript = `#!/usr/bin/env node
const input = JSON.parse(require('fs').readFileSync(0, 'utf-8'));
console.log(JSON.stringify({
  cancel: false,
  contextModification: "Tool succeeded: " + input.postToolUse.success
	...
	describe("Hook Discovery", () => {
		it("should generate Windows PowerShell bridge files with real newlines", async () => {
			const hooksDir = path.join(tempDir, ".codevibe", "hooks")
  ...
  contextModification: hasAllFields ? "All fields present" : "Missing fields"
	...
	describe("Global Hooks", () => {
		...
		beforeEach(async () => {
			...
			workspaceHooksDir = path.join(tempDir, ".codevibe", "hooks")
...
console.log(JSON.stringify({
  ...
  contextModification: "GLOBAL: Context added"
			...
			const workspaceHookPath = path.join(tempDir, ".codevibe", "hooks", "PreToolUse")
