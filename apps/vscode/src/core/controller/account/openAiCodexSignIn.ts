...
import { openAiCodexOAuthManager, safeOpenAiCodexErrorSummary } from "@/integrations/openai-codex/oauth"
	...
	try {
		...
		openAiCodexOAuthManager
			...
			.catch((error) => {
				...
				Logger.error(`[openAiCodexSignIn] OAuth callback failed: ${errorMessage}`)
		...
		Logger.error(`[openAiCodexSignIn] Failed to start OAuth flow: ${safeOpenAiCodexErrorSummary(error)}`)
		openAiCodexOAuthManager.cancelAuthorizationFlow()
		throw error
	}
