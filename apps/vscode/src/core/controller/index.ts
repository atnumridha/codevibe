...
import type {
	CodeVibeCompatibilityStatus,
	ExtensionState,
	Platform,
} from "@shared/ExtensionMessage";
import type { HistoryItem } from "@shared/HistoryItem";
import {
	normalizeMcpMarketplaceCatalog,
...
type PostStateToWebviewOptions = {
...
function summarizeCursorSandboxRuntimePolicy(
	...
	return {
		...
		configSource:
			policy.configSource ??
			(policy.source === "cursor-sandbox" ? "cursorCompatibility" : "codie"),
...
export class Controller {
	...
	private backgroundAgentTaskRecords = new Map<
		string,
	...
	*/
	...
	async handleSignOut() {
		try {
			...
			HostProvider.window.showMessage({
				...
				message: "Successfully logged out of Codie",
			});
		} catch (_error) {
			HostProvider.window.showMessage({
				type: ShowMessageType.INFORMATION,
	...
	async initTask(
		...
		const cursorCompatibilityEnabled = getCodeVibeConfigurationValue<boolean>(
			"cursorCompatibility.enabled",
			true,
		);
		const getCursorSafeBrowserEvaluateEnabled = () => {
			return (
				getCodeVibeConfigurationValue<boolean>(
					"cursorCompatibility.enabled",
					true,
				) &&
				getCodeVibeConfigurationValue<boolean>(
		...
		const cursorSandboxPolicy = await resolveCursorSandboxPolicy({
			...
			policySetting: getCodeVibeConfigurationValue<string>(
	...
	async handleCursorAutomationIngest(
		...
		const result = await ingestCursorAutomationEvents(
	...
	async handleCursorPluginAdd(request: {
		...
		try {
			...
			const cwd =
				workspaceManager?.getPrimaryRoot()?.path ||
				(await getCwd(getDesktopDir()));
			const result = await installPlugin({
				...
				io: {
					writeln: (message = "") =>
						Logger.info(`Codie plugin install: ${message}`),
					writeErr: (message) =>
						Logger.warn(`Codie plugin install: ${message}`),
				},
			});
			await HostProvider.window.showMessage({
				type: ShowMessageType.INFORMATION,
				message: `Installed Codie plugin from ${result.source}.`,
				options: {
					items: [],
					detail: [
						`Path: ${result.installPath}`,
						`Entries: ${result.entryPaths.length}`,
					].join("\n"),
				},
			});
			return result;
			...
			Logger.error("Failed to install Codie plugin:", error);
			await HostProvider.window.showMessage({
				type: ShowMessageType.ERROR,
				message: `Failed to install Codie plugin: ${message}`,
			});
			throw error;
		}
	}
	...
	getBackgroundAgentTaskRecords(): BackgroundAgentTaskRecord[] {
		return normalizeBackgroundAgentTaskRecords([
			...this.backgroundAgentTaskRecords.values(),
		]);
	}
	...
	private hydrateBackgroundAgentTaskRecords() {
		...
		this.backgroundAgentTaskRecords = new Map(
	...
	async handleCursorBackgroundAgentLaunch(
	...
	private async fetchMcpMarketplaceFromApi(): Promise<McpMarketplaceCatalog> {
		...
		let items: McpMarketplaceItem[] = normalizeMcpMarketplaceCatalog({
			items: response.data || [],
		}).items;
	...
	async postStateToWebview(options: PostStateToWebviewOptions = {}) {
		const state = await this.getStateToPostToWebview(options);
		await sendStateUpdate(state);
	}

	private async getOpenAiCodexBackendModelsForState(
		isAuthenticated: boolean,
		options: PostStateToWebviewOptions = {},
		...
		if (
			!options.skipOpenAiCodexBackendModelsRefresh &&
			!this.openAiCodexBackendModelsPromise
		) {
			this.openAiCodexBackendModelsPromise = (async () => {
				...
				.catch(async (error) => {
					const { isOpenAiCodexAuthFailure, safeOpenAiCodexErrorSummary } =
						await import("@/integrations/openai-codex/oauth");
					Logger.error(
						`[Codie hosted models] Failed to list backend models: ${safeOpenAiCodexErrorSummary(error)}`,
					);
					const models =
						this.openAiCodexBackendModelsCache?.models ??
			...
			void this.openAiCodexBackendModelsPromise
				.then(() =>
				...
				.catch((error) => {
					Logger.error(
						`[Codie hosted models] Failed to post backend model refresh state: ${
	...
	async getStateToPostToWebview(
		...
		let welcomeViewCompleted = !!this.stateManager.getGlobalStateKey(
		...
		// Check Codie authentication status.
		const { openAiCodexOAuthManager } = await import(
			"@/integrations/openai-codex/oauth"
		);
		const openAiCodexIsAuthenticated =
			await openAiCodexOAuthManager.hasUsableCredentials();
		if (openAiCodexIsAuthenticated && !welcomeViewCompleted) {
			this.stateManager.setGlobalState("welcomeViewCompleted", true);
			await this.stateManager.flushPendingState();
			welcomeViewCompleted = true;
		}
		const openAiCodexBackendModels =
			await this.getOpenAiCodexBackendModelsForState(
				openAiCodexIsAuthenticated,
				options,
			);
		const compatibilityEnabled = getCodeVibeConfigurationValue<boolean>(
			"cursorCompatibility.enabled",
			true,
		);
		const compatibilitySandboxPolicy = getCodeVibeConfigurationValue<
			"prompt" | "workspace" | "readOnly" | "disabled"
		>("cursorCompatibility.sandboxPolicy", "prompt");
		const sandboxWorkspaceRoot =
			this.workspaceManager?.getPrimaryRoot()?.path ||
			(await getCwd(getDesktopDir()));
			...
			: createInactiveSandboxRuntimeSummary({
					status:
						compatibilityEnabled && compatibilitySandboxPolicy !== "disabled"
							? "missing"
							: "disabled",
					configPath: sandboxWorkspaceRoot
						? resolveCursorSandboxConfigPath(sandboxWorkspaceRoot)
						: undefined,
					workspaceRoot: sandboxWorkspaceRoot,
				});
		const compatibilitySafeBrowserEvaluateEnabled =
			getCodeVibeConfigurationValue<boolean>(
				"cursorCompatibility.safeBrowserEvaluate.enabled",
				false,
			);
		const effectiveBrowserSettings = getEffectiveBrowserSettings(
			browserSettings,
			{
				cursorCompatibilitySafeBrowserEvaluateEnabled:
					compatibilitySafeBrowserEvaluateEnabled,
			},
		);
		const compatibilityStatus: ExtensionState["compatibilityStatus"] = {
			enabled: compatibilityEnabled,
			deepLinksEnabled: getCodeVibeConfigurationValue<boolean>(
			...
			effectiveBrowserEvaluateEnabled:
				effectiveBrowserSettings.allowBrowserEvaluate,
			openAiCodexAuthSource: getCodeVibeConfigurationValue<
				"codexHome" | "vscodeSecret" | "auto"
			>("openAiCodex.authSource", "auto"),
		...
		return {
			...
			welcomeBanners,
			openAiCodexIsAuthenticated,
			...(openAiCodexBackendModels
				? { openAiCodexModels: openAiCodexBackendModels }
				: {}),
			compatibilityStatus,
		};
	}
