...
const CLINE_OUTPUT_CHANNEL = vscode.window.createOutputChannel("CodeVibe")

// Appends a log message to the CodeVibe output channel.
export async function debugLog(request: StringRequest): Promise<Empty> {
	CLINE_OUTPUT_CHANNEL.appendLine(request.value)
	return Empty.create({})
}

// Register the CodeVibe output channel within the VSCode extension context.
