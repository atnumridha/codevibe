...
- start a new session from an initial prompt — workspace/provider/model are reused from the most recent session, or `CODEVIBE_PROVIDER` / `CODEVIBE_MODEL` env vars
...
This starts the Vite webview server on <http://127.0.0.1:5173> and the hub dashboard on <http://127.0.0.1:8787>. Open the dashboard URL; the served page loads webview modules from Vite, so changes under `src/webview/src` hot reload without rebuilding. Use `CODEVIBE_HUB_WEBVIEW_DEV_PORT` or `CODEVIBE_HUB_WEBVIEW_DEV_HOST` to change the Vite bind address.

To start a brand-new session, the dashboard needs to know which provider and model to use. It picks them up automatically from the most recent session on the hub. If there are no recent sessions, set `CODEVIBE_PROVIDER` and `CODEVIBE_MODEL` in the environment before running.
...
| `CODEVIBE_HUB_DASHBOARD_PORT` | `8787` | Dashboard HTTP/WebSocket port. |
...
| `CODEVIBE_PROVIDER` | unset | Fallback provider id when no recent session is available to copy from. |
| `CODEVIBE_MODEL` | unset | Fallback model id when no recent session is available to copy from. |
...
```bash
HOST=0.0.0.0 \
CODEVIBE_HUB_DASHBOARD_PORT=8787 \
...
1. Detaches the dashboard clients from the current hub.
